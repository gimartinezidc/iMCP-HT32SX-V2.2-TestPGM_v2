...
from lib.stlinkusb import reset_usb_device
...
list of available actions:
  ...
  flash[:erase][:{addr}][:verify]:{file.srec}     erase + flash SREC file + verify
...
class PyStlink():
    ...
    def cmd_flash(self, params):
        firm_file = self.file_name #self.get_firm_file()
        credentials_file = self.file_name #self.get_credentials_file()

        erase = False
        if params[0] == 'erase':
            params = params[1:]
            if not params and not self.addr_flag and not self.firm_flag:
                self._driver.flash_erase_all()
                return
            elif self.addr_flag:
                size = self.get_file_size(credentials_file)
                self._driver.eeprom_erase_sector(0x08080000, size)
                #self.addr_flag = 0
                return
            elif self.firm_flag:
                size = self.get_file_size(firm_file)
                self._driver.flash_erase_sector(0x08000000, size)
                #self.firm_flag = 0
                return
            erase = True
        ...
        for addr, data in mem:
            if addr is None:
                addr = start_addr
            if self.firm_flag:
                self._driver.flash_write(addr, data, erase=erase, verify=verify,
                                         erase_sizes=self._mcus_by_devid['erase_sizes'])
                self.firm_flag = 0
            elif self.addr_flag:
                self._driver.eeprom_write(addr, data, erase=erase, verify=verify,
                                         erase_sizes=self._mcus_by_devid['erase_sizes'])
                self.addr_flag = 0
